...
class TomoModel(L.LightningModule):
  def __init__(self, inputsize, learning_rate, outputsize,fc_layer_size=128,n_layer=3, activation='ReLU'):
    super().__init__()
    ...
    self.layers.append(nn.Linear(inputsize, fc_layer_size))
    self.activations.append(self._get_activation())
    
    # Aggiungere layer successivi
    for _ in range(1, n_layer):
        self.layers.append(nn.Linear(fc_layer_size, fc_layer_size))
        self.activations.append(self._get_activation())

    # Ultimo layer di output
    self.output_layer = nn.Linear(fc_layer_size, outputsize)
    #######
    ######
